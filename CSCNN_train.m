function [net, state] = CSCNN_train(net, imdb, varargin)
...
modelPath = @(ep) fullfile(opts.models_dir, sprintf([opts.modelName,'-epoch-%d.mat'], ep));
...
%     Matrix = net.layers{1}.weights{1};
%     Mat = zeros(102,1024);
%     for ji = 1:size(Matrix,4)
%        Mat(ji,:) = reshape(Matrix(:,:,:,ji),1,[]);
%     end
%     Matrix = Mat;
    
%     if(epoch == start+1)
%        Demo_Image_CS_Recovery_via_GSR('datasets\BSDS100\GT','GSR_Result','GSR_Result2',0.1,Matrix,50);
%     end
    
    % imdb = GenerateData_model_64_25_Res_Bnorm_Adam(32,300,10,40);
    opts.train = find(imdb.set==1);
    opts.test  = find(imdb.set==2);
