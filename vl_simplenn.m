  ...
  switch l.type
    case 'conv'
      if(i  == 1)
         ...
         l.weights{1} = Matrix_Sparse(0.8, l.weights{1}, []);
         % normalization constraint
         l.weights{1} = Matrix_orth(l.weights{1},[]);
         net.layers{i}.weights{1} = l.weights{1};
      end
      if(gpu == 1)
         res(i).x = gpuArray(res(i).x);
      ...
      case 'dep_conv'
          ...
          my_global2 = l.weights{1};
          %l.weights{1} = Matrix_orth(l.weights{1},[]);
          l.weights{1} = Matrix_orth_dep(l.weights{1},[]);
          net.layers{i}.weights{1} = l.weights{1};
          ...
          for dim = 1:dims
              ...
              %map_temp = gpuArray(map_temp);
              res_temp = vl_nnconv(map_temp, l.weights{1}(:,:,dim,:), [], ...
                  'pad', l.pad, ...
                  'stride', l.stride, ...
                  l.opts{:}, ...
                  cudnn{:}) ;
    ...
    case 'loss'
      res(i+1).x = vl_nnloss_cui(res(i).x, l.class) ;
	...
	case 'bcs_init_rec'
	  res(i+1).x = bcs_initialRec(res(i).x, l.dims);
    ...
    switch l.type
      ...
      case 'conv'
        ...
        if(gpu == 1)
           ...
           l.weights{1} = gpuArray(l.weights{1});
           l.weights{2} = gpuArray(l.weights{2});
           res(i+1).dzdx = gpuArray(res(i+1).dzdx);
        end
		...
		% backward for the normalization contraint
        if(i == 1)
            dzdw{1} = gather(dzdw{1});
            maxs = max(max(max(max(abs(dzdw{1})))));
            maxs = maxs * 10;
            dzdw{1} = dzdw{1}/maxs;
            dzdx = Matrix_orth(my_global,dzdw{1});
            dzdw{1} = dzdx;
            maxs = max(max(max(max(abs(dzdw{1})))));
            maxs = maxs * 10;
            dzdw{1} = dzdw{1}/maxs;
%             for ii = 1:size(dzdw{1},4)
%                 dzdw_temp = dzdw{1}(:,:,:,ii);
%                 dzdw_temp = squeeze(dzdw_temp);
%                 rand1 = round(rand()*31+1);
%                 rand2 = round(rand()*31+1);
%                 dzdw_value = dzdw_temp(rand1,rand2);
%                 dzdw_temp = dzdw_temp*0;
%                 dzdw_temp(rand1,rand2) = dzdw_value;
%                 dzdw{1}(:,:,:,ii) = dzdw_temp;
%             end
      ...
      case 'dep_conv'
        ...
        for dim = 1:dims
            ...
            [dzdx_temp, dzdw_temp1, dzdw_temp2] =  ...
                vl_nnconv(map_temp, l.weights{1}(:,:,dim,:), [], res(i+1).dzdx(:,:,dim,:), ...
                'pad', l.pad, ...
                'stride', l.stride, ...
                l.opts{:}, ...
                cudnn{:}) ;
            res(i).dzdx(:,:,dim,:) = dzdx_temp;
        ...
        maxs = maxs * 10;
        dzdw{1} = dzdw{1}/maxs;
        dzdx = Matrix_orth_dep(my_global2,dzdw{1});
        dzdw{1} = dzdx;
        maxs = max(max(max(max(abs(dzdw{1})))));
        maxs = maxs * 10;
        dzdw{1} = dzdw{1}/maxs;
        ...
        %                 dzdw{1}(:,:,:,ii) = dzdw_temp;
        %             end
        net.layers{i}.weights{1} = my_global2;
	  ...
	  case 'bcs_init_rec'
	    res(i).dzdx = bcs_initialRec(res(i).x, l.dims,res(i+1).dzdx);

      case 'loss'
        res(i).dzdx = vl_nnloss_cui(res(i).x, l.class, res(i+1).dzdx) ;
